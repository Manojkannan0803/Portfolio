...
def show_map(M, start=None, goal=None, path=None):
    G = M._graph  # Assuming M._graph is a NetworkX graph
    
    # Initialize the edge_trace with lists for x and y
    edge_trace = go.Scatter(
        x=[],  # Ensure this is a list
        y=[],  # Ensure this is a list
        line=dict(width=0.5, color='#888'),
        hoverinfo='none',
        mode='lines'
    )

    # Loop over the edges to append the coordinates
    for edge in G.edges():
        x0, y0 = G.nodes[edge[0]]['pos']
        x1, y1 = G.nodes[edge[1]]['pos']
        
        # Ensure we are extending the lists properly
        edge_trace['x'].extend([x0, x1, None])  # Use dictionary-style key access
        edge_trace['y'].extend([y0, y1, None])  # Use dictionary-style key access

    # Initialize node_trace with lists for x, y, and text
    node_trace = go.Scatter(
        x=[],  # Ensure this is a list
        y=[],  # Ensure this is a list
        text=[],  # List for node text
        mode='markers',
        hoverinfo='text',
        marker=dict(
            showscale=False,
    ...
    # Add node positions to node_trace
    for node in G.nodes():
        x, y = G.nodes[node]['pos']
        node_trace['x'].append(x)  # Use dictionary-style key access
        node_trace['y'].append(y)  # Use dictionary-style key access

    # Add node color and hover text
    for node in G.adjacency():
        ...
        if node == start:
            ...
            color = 1
        node_trace['marker']['color'].append(color)  # Use dictionary-style key access
        node_trace['text'].append(f"Intersection {node}")
